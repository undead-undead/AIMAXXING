...
/// - `filename_prefix`: Prefix for log files (e.g. "aimaxxing.log")
...
pub fn init_logging(directory: &str, filename_prefix: &str, level: &str) -> Result<()> {
    ...
    let file_appender = RollingFileAppender::builder()
        ...
        .build(directory)
        .map_err(|e| {
            brain::error::Error::Internal(format!("Failed to create log appender: {}", e))
        })?;
    ...
    registry
        .with(console_layer)
        .try_init()
        .map_err(|e| brain::error::Error::Internal(format!("Failed to init tracing: {}", e)))?;
