...
impl super::Connector for DingTalkConnector {
    ...
    async fn send(&self, message: OutboundMessage) -> Result<()> {
        ...
        if !token_resp.status().is_success() {
            return Err(crate::error::Error::Internal(format!("DingTalk token fetch failed: {}", token_resp.status())));
        }
        let token_data: serde_json::Value = token_resp.json().await?;
        let access_token = token_data["access_token"].as_str()
            .ok_or_else(|| crate::error::Error::Internal("Failed to extract DingTalk access_token".to_string()))?;
        ...
        if message.chat_id.is_empty() {
            return Err(crate::error::Error::Internal("DingTalk message chat_id (open_conversation_id/userid) is missing".to_string()));
        }
        ...
        if !send_resp.status().is_success() {
            ...
            return Err(crate::error::Error::Internal(format!("DingTalk send failed: {}", error_body)));
        }
