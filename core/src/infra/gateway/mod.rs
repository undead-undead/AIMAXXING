    ...
    pub async fn run(self) -> crate::error::Result<()> {
        ...
        if !canvas_path.exists() {
            fs::create_dir_all(&canvas_path)
                .map_err(|e| crate::error::Error::Internal(format!("Failed to create canvas dir: {}", e)))?;
        ...
        if self.config.log_to_canvas {
            ...
            fs::create_dir_all(&log_dir).map_err(|e| crate::error::Error::Internal(format!("Failed to create logs dir: {}", e)))?;
