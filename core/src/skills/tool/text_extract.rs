...
use crate::agent::message::{Message, Role, Content, ContentPart, ImageSource};
...
async fn ocr_via_provider(provider: &dyn Provider, path: &str, model: &str) -> anyhow::Result<String> {
    ...
    while let Some(choice) = stream.next().await {
        match choice? {
            crate::agent::streaming::StreamingChoice::Message(text) => full_text.push_str(&text),
            crate::agent::streaming::StreamingChoice::Done => break,
            _ => {}
        }
    }
